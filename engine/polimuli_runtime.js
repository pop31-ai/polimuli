...
var Polimuli = (() => {
  ...
  function play(data, id) {
    ...
    const board = Object.assign({
      ...
      applause: false,          // рукоплескания, когда доска дописана
      applauseStep: 0.16,       // золотой шаг волн аплодисментов (≈ φ-ритм)
      applauseNote: '',         // плашка-благодарность (пусто = выкл)
      applauseBecome: '',       // «кем станешь» — образ результата труда (пусто = выкл)
    }, st.board || {});
    ...
    const applause = new Map();   // акт -> t старта рукоплесканий
    const applauseSeen = new Set(); // акт.волна -> спаун выполнена
    ...
    function drawBoard(t) {
      ...
      // Рукоплескания по золотому сечению — невербальное подтверждение зрителю,
      // который «вдохнул» знание. Когда доска дописана (последняя строка
      // завершена рукой) — из-под доски всплеск 👏 с золотым ритмом спауна.
      // Включается полем board.applause; без него — прежнее поведение.
      if (board.applause) {
        const last = lines.length - 1;
        const lineStart = cur.at + last * board.step;
        const handW = Math.max(0.6, board.step * 0.85);
        if (t >= lineStart + handW) {
          const bx0 = board.x + board.gutter, bx1 = board.x + board.w - board.gutter;
          const by0 = board.y + board.h - board.gutter;
          if (!applause.get(cur)) applause.set(cur, t);
          const start = applause.get(cur);
          const PHI = 1.6180339887;
          // «полиномиально и золотым сечением»: волны аплодисментов идут
          // ступенями, кратными φ — как живой зал, а не одинаковая циклограмма
          const slot = Math.floor((t - start) / board.applauseStep);
          for (let k = 0; k <= slot; k++) {
            const tag = cur + '.' + k;
            if (applauseSeen.has(tag)) continue;
            applauseSeen.add(tag);
            const n = Math.round(6 + k * 3);
            for (let i = 0; i < n; i++) {
              const u = i / Math.max(1, n - 1);
              const x = bx0 + (bx1 - bx0) * u;
              const emoji = (i % 3 === 2) ? (i % 7 === 5 ? '✨' : '🙂') : '👏';
              Parts.spawn({
                x, y: by0,
                vx: (u - 0.5) * 90 + (Math.random() - 0.5) * 30,
                vy: -70 - Math.random() * 60,
                life: 0.8 + Math.random() * 0.7, size: 16 + Math.random() * 8,
                emoji, gravity: 130, alpha: 0.9,
              });
            }
          }
          // образ результата труда — «кем ты станешь» (золотое сечение —
          // просто инструмент композиции, цель — живой образ становления)
          if (board.applauseNote || board.applauseBecome) {
            const fade = Math.min(1, (t - start) / board.applauseStep);
            const ax = bx0 + (bx1 - bx0) / PHI;   // φ-точка: инструмент, не догма
            ctx.save();
            ctx.globalAlpha = fade;
            ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
            ctx.fillStyle = board.titleColor;
            if (board.applauseBecome) {
              ctx.font = `bold ${Math.round(board.font * 1.15)}px "Segoe UI", Arial, sans-serif`;
              ctx.fillText(board.applauseBecome, ax, by0 - board.rowH * 0.9);
            }
            if (board.applauseNote) {
              ctx.font = `${Math.round(board.font * 0.9)}px "Segoe UI", Arial, sans-serif`;
              ctx.fillText(board.applauseNote, ax, by0 - board.rowH * 0.9
                            - (board.applauseBecome ? board.font * 1.3 : 0));
            }
            ctx.restore();
          }
        }
